...
// 3D Tilt Hero
const TiltSection = styled(HeroSection)`
  background: #0a0a12;
  perspective: 1000px;
`;
...
const CurvedStepCard = styled.div`
  position: relative;
  text-align: center;
  z-index: 2;
...
const CurvedStepNumber = styled.div`
  width: 70px;
  height: 70px;
  margin: 0 auto 20px;
  background: ${({ $color }) => $color};
...
const FloatingCardNumber = styled.div`
  position: absolute;
  top: -15px;
  right: 20px;
  width: 36px;
...
export default function DemoPage() {
  const [activeHero, setActiveHero] = useState('purple-glow');
  ...
  const getMagneticPosition = (baseX, baseY) => {
  ...
  const selectButton = (category, value) => {
